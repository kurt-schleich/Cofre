...
class TestCofre(unittest.TestCase):
    ...
    def testInserirMoedaCofreQuebrado(self):
        cofre = Cofre(25)
        cofre.quebrar()
        self.assertFalse(cofre.add_M(Moeda.M50), "Nao deve ser possivel adicionar uma moeda em um cofre quebrado")

    def testInserirItemCofreQuebrado(self):
        cofre = Cofre(25)
        cofre.quebrar()
        teste = Item("Item teste", 10)
        self.assertFalse(cofre.add_I(teste), "Nao deve ser possivel adicionar uma moeda em um cofre quebrado")

    def testInserirItemVolumoso(self):
        cofre = Cofre(25)
        arvore = Item("Arvore", 1000)
        self.assertFalse(cofre.add_I(arvore), "Nao deve ser possivel adicionar um item com volume maior que o do cofre")

    def testInserirItem(self):
        cofre = Cofre(25)
        passaporte = Item("Passaporte", 5)
        self.assertTrue(cofre.add_I(passaporte), "Tem espaco sobrando, entao deve ser possivel adicionar o item")
    ...
    def testInserirMoeda(self):
        cofre = Cofre(10)
        self.assertTrue(cofre.add_M(Moeda.M10), "Tem espaco sobrando, entao deve ser possivel adicionar o item")
    ...
    def testInserirItemEmCofreCheio(self):
        ...
        self.assertFalse(cofre.add_M(Moeda.M10), "Nao deve ser possivel adicionar algo em um cofre cheio")
        self.assertFalse(cofre.add_I(passporte), "Nao deve ser possivel adicionar algo em um cofre cheio")
